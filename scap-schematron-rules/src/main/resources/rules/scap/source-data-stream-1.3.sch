 ...
 xmlns:xsl="http://www.w3.org/1999/XSL/Transform" queryBinding="xslt2">
  ...
  <sch:pattern id="scap-general">
    ...
    <sch:rule id="scap-general-scap-content" context="scap:data-stream">
      ...
      <sch:assert id="scap-general-xccdf-check-content-ref-multiple-patch-id-req" test="every $m in ds:checklists/ds:component-ref satisfies every $n in xcf:get-component($m)//xccdf:Rule[starts-with(@id,'xccdf_') and ends-with(@id,'_rule_security_patches_up_to_date')]//xccdf:check-content-ref[not(xcf:is-external-ref($m/cat:catalog, @href) cast as xsd:boolean)] satisfies if ($n/parent::node()[@multi-check eq 'true']) then count(xcf:get-component(xcf:get-component-ref($m/cat:catalog, $n/@href))//oval-def:definition[@class eq 'patch']) gt 1 else true()">SRC-169-1|scap:data-stream <sch:value-of select="@id"/></sch:assert>
