  ...
  <xsl:template match="ds:data-stream">
    ...
    <xsl:for-each select="ds:checklists/ds:component-ref">
      ...
      <xsl:for-each
        ...
        <xsl:if
          test="not(not(matches($n/ancestor::xccdf:Benchmark//xccdf:Value[@id eq $n/@value-id]/@type, '^boolean$')) or (every $o in $n/following-sibling::xccdf:check-content-ref satisfies matches(xcf:get-component(xcf:get-component-ref($m/cat:catalog, $o/@href))//*[@id eq $n/@export-name]/@datatype, '^boolean$')))">
