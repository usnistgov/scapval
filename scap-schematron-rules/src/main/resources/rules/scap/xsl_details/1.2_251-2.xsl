...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
  ...
  <xsl:template match="ds:data-stream">
    ...
    <xsl:for-each select="ds:checklists/ds:component-ref">
      ...
      <xsl:for-each
        select="xcf:get-component($m)//xccdf:ident[matches(@system,'^(http://cce.mitre.org|http://cve.mitre.org|http://cpe.mitre.org)$')]">
        <xsl:variable name="n" select="."/>
        <xsl:for-each
          select="$n/ancestor::xccdf:Rule[1]/xccdf:check[@system eq 'http://oval.mitre.org/XMLSchema/oval-definitions-5']/xccdf:check-content-ref[exists(@name)]">
          <xsl:variable name="o" select="."/>
          <xsl:if
            test="not(exists(xcf:get-component(xcf:get-component-ref($m/cat:catalog, $o/@href))//oval-def:definition[@id eq $o/@name]//oval-def:reference[matches(@source,xcf:ident-mapping($n/@system)) and $n eq @ref_id]))">
