...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <sch:pattern id="scap-result-general">
    ...
    <sch:rule id="scap-result-general-xccdf-test-result" context="xccdf:TestResult">
      ...
      <sch:assert id="scap-result-general-xccdf-test-result-target-address-content" test="every $m in xccdf:target-address satisfies matches($m, '^(([0-9]|[1-9][0-9]|1([0-9][0-9])|2([0-4][0-9]|5[0-5]))\.){3}([0-9]|[1-9][0-9]|1([0-9][0-9])|2([0-4][0-9]|5[0-5]))(/([0-9]|[12][0-9]|3[0-2]))?$') or matches($m, '^(([0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,7}:|([0-9a-fA-F]{1,4}:){1,6}:[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,5}(:[0-9a-fA-F]{1,4}){1,2}|([0-9a-fA-F]{1,4}:){1,4}(:[0-9a-fA-F]{1,4}){1,3}|([0-9a-fA-F]{1,4}:){1,3}(:[0-9a-fA-F]{1,4}){1,4}|([0-9a-fA-F]{1,4}:){1,2}(:[0-9a-fA-F]{1,4}){1,5}|[0-9a-fA-F]{1,4}:(:[0-9a-fA-F]{1,4}){1,6}|:((:[0-9a-fA-F]{1,4}){1,7}|:)|([0-9a-fA-F]{1,4}:){1,4}:(([0-9]|[1-9][0-9]|1([0-9][0-9])|2([0-4][0-9]|5[0-5]))\.){3}([0-9]|[1-9][0-9]|1([0-9][0-9])|2([0-4][0-9]|5[0-5]))|::([fF]{4}(:0{1,4})?:)?(([0-9]|[1-9][0-9]|1([0-9][0-9])|2([0-4][0-9]|5[0-5]))\.){3}([0-9]|[1-9][0-9]|1([0-9][0-9])|2([0-4][0-9]|5[0-5])))(%[0-9a-zA-Z._~-]+)?(/([0-9]|[1-9][0-9]|1[01][0-9]|12[0-8]))?$')">RES-136-2</sch:assert>
    ...
    <sch:rule id="scap-result-general-oval-results" context="oval-res:oval_results">
      <sch:assert id="scap-result-oval-results-result-type" test="(count(oval-res:directives/*) eq 6) and (every $m in oval-res:directives/* satisfies(($m/@reported eq 'true') and (if(exists($m/following-sibling::*)) then (if ($m/@content eq 'thin') then ($m/following-sibling::*[1]/@content eq 'thin' or not(exists($m/following-sibling::*[1]/@content))) else if ($m/@content eq 'full' or not(exists($m/@content))) then ($m/following-sibling::*[1]/@content eq 'full' or not(exists($m/following-sibling::*[1]/@content))) else false()) else true())))">RES-179-1</sch:assert>
       <sch:assert id="scap-result-oval-results-sys-with-or-wo-char" test="if( oval-res:directives/*[1]/@content eq 'full' or not(exists(oval-res:directives/*[1]/@content))) then not(exists(.//oval-sc:oval_system_characteristics/oval-sc:system_data) and not(exists(.//oval-sc:oval_system_characteristics/oval-sc:collected_objects))) else true()">RES-181-1</sch:assert>
    </sch:rule>
