...
import gov.nist.secauto.scap.validation.candidate.ScapFolderInspector;
...
public class ScapFolderInspectorTest {

  private final ScapFolderInspector inspector = new ScapFolderInspector();
  ...
  @Test
  public void testScapFolder() throws Exception {
    doTestScapFolder("classpath:src/test/resources/candidates/scap-11/source-folder");
    doTestScapFolder("classpath:src/test/resources/candidates/scap-11/source-folder-extra-files");
  }

  private void doTestScapFolder(final String path) throws Exception {
    ...
    assertEquals(false, candidate.isUnrecognized());

    final List<SCAP11Components> contentTypes = candidate.getScapContentTypes();

    assertNull(candidate.getScapVersion());
    assertNull(candidate.getXccdfVersion());

    assertTrue(contentTypes.contains(SCAP11Components.XCCDF_BENCHMARK));
    assertTrue(contentTypes.contains(SCAP11Components.OVAL_COMPLIANCE));
    assertTrue(contentTypes.contains(SCAP11Components.CPE_DICTIONARY));
  }
