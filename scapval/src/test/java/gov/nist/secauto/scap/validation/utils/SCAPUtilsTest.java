  ...
  public void getOVALComponentsFromSCAPContent() throws Exception {
    ...
    final File datastream12 = new File(
        new URL("classpath:src/test/resources/candidates/scap-12/scap_gov.nist_USGCB-Windows-XP-firewall.xml")
            .getFile());
    List<Element> OVALElements12
        = SCAPUtils.getOVALComponentsFromSCAPContent(new JDOMDocument(datastream12), SCAPVersion.V1_2);
    Assert.assertEquals(OVALElements12.size(), 2);

    final File datastream13 = new File(
        new URL("classpath:src/test/resources/candidates/scap-13/source_data_stream_collection_sample.xml").getFile());
    List<Element> OVALElements13
        = SCAPUtils.getOVALComponentsFromSCAPContent(new JDOMDocument(datastream13), SCAPVersion.V1_3);
