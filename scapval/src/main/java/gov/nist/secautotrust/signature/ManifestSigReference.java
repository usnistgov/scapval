...
class ManifestSigReference extends DefaultSigReference implements SigObject {
  ...
  public ManifestSigReference(String uri, List<DefaultSigReference> sigReferences,
    ...
    super(SignatureRelationship.ENVELOPING, uri, canonicalizationType, (List<XPathType>) null, (Element) null,
        Manifest.TYPE, hashType);
  ...
  public XMLObject createObject(Signature signer) throws Exception {
    ...
    List<Reference> references = new ArrayList<Reference>(sigReferences.size());
    for (DefaultSigReference r : sigReferences) {
      references.add(r.createReference(signer));
    }
    Manifest manifest = factory.newManifest(references, getUri().substring(1));
    return factory.newXMLObject(Collections.singletonList(manifest), (String) null, (String) null, (String) null);
  }
