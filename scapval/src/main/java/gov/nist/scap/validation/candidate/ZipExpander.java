...
public class ZipExpander {
            ...
            if (zipInputStream != null) {
                try {
                    ...
                    log.error(String.format(
                        "Unable to close entry in %s",
                        compressedFile.getAbsolutePath()), e);
                }
                try {
    ...
    private void writeFile(
        ...
        try {
            ...
            log.error(
                String.format(
                    "FileNotFoundException while writing %s",
                    newFile.getAbsolutePath()),
                e);
        } catch (IOException e) {
            log.error(
                String.format(
                    "IOException writing %s",
                    newFile.getAbsolutePath()),
                e);
        } finally {
    ...
    private File getTargetDirectory(final File compressedFile, final File targetDir) {
        ...
        if (pos < 1) {
            throw new IllegalStateException(String.format(
                "Unable to construct target unzip directory from %s",
                compressedFilePath));
        }
        ...
        while (target.exists()) {
            ...
            target =
                new File(targetDir, targetPath + String.valueOf(++index));
            if (index > 999) {
                throw new IllegalStateException(
                    "Unable to generate target unzip directory");
            }
        }
        return target;
    }
