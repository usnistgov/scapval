...
public class ContentCombiner {
     ...
     * SCAPVal. This is used for SCAP 1.1 content only. Including the sourceDS is optional otherwise
     ...
     * @param sourceDS
     *            an XMLDocument of source content that will be included in the combined result
    ...
    public static XMLDocument combineSCAP11(File scapContentsDir, String scapUseCase, int maxDownloadSize,
            boolean isOnline, XMLDocument sourceDS)
            throws SCAPException, IOException, DocumentException, JDOMException {
        ...
        for (Map.Entry<String, JDOMDocument> scapComponentDoc : scapComponentDocs.entrySet()) {
            ...
            String systemID = new File(scap11BundleDir.getAbsolutePath() + FileUtils.PATH_SEPERATOR + filename).toURI()
                    .toString();
    ...
    /**
     * Includes a SCAP source data-stream into an ARF file using the arf:report-requests element.
     * This is used for SCAP 1.2 and 1.3 content only.
     *
     * @param arfFile
    ...
    public static void mergeARFWithDS(XMLDocument arfFile, XMLDocument dsFile, File combinedOutput)
        ...
        log.info("Attempting to embed the source data-steam: " + dsFilePath + " into the ARF file: " + arfFilePath
                + " and " + "creating file: " + combinedOutputPath);
        ...
        Element reportRequests = newCombined.getJDOMDocument(true).getRootElement().getChild("report-requests",
        ...
        // add the datastream
        Document merged
                = reportRequestContent.addContent(dsFile.getJDOMDocument().getRootElement().detach()).getDocument();
            ...
            if (component.getAttribute("href", NS_XLINK.getNamespace()) != null) {
                ...
                if (xlinkHrefAttribute.getValue().startsWith("http")) {
                    ...
                    try {
                        ...
                        log.info("Invalid http/https location for component-ref with id " + idAttribute.getValue()
                                + " and " + "URL " + xlinkHrefAttribute.getValue() + " Will not "
                                + "download the remote component." + e.getMessage());
                        ValidationNotes.getInstance()
                                .createValidationNote("Invalid http/https location for component-ref with id "
                                        + idAttribute.getValue() + " and " + "URL " + xlinkHrefAttribute.getValue()
                                        + " Will not " + "download the remote component.");
                        continue;
                    }
                    ...
                    try {
                        ...
                        log.info("Cannot access the directory of the validation target: " + relativeDir + " " + "and "
                                + "URL " + xlinkHrefAttribute.getValue() + " Will not include this file " + "in"
                                + " validation." + e.getMessage());
                        ValidationNotes.getInstance()
                                .createValidationNote("Cannot access the directory of the validation target: "
                                        + relativeDir + " " + "and " + "URL " + xlinkHrefAttribute.getValue()
                        ...
                        ValidationNotes.getInstance().createValidationNote(
                                "Cannot read file at location specified for component-ref with id "
                                        + idAttribute.getValue() + " and " + "" + "" + "" + "URL " + "file:"
                                        + relativeDir + fileNameToMerge + " Will not include this file in" + " "
                                        + "validation.");
                        continue;
                    }
                } else {
                ...
                try {
                    log.info("Found a remote component-ref with id " + idAttribute.getValue() + " and " + "URL" + " "
                            + xlinkHrefAttribute.getValue() + " will attempt to acquire and " + "include in "
                            + "validation.");
                    ...
                    if (idAttribute.getValue().matches("scap_[^_]+_cref_.+")) {
                        ...
                        ValidationNotes.getInstance().createValidationNote(
                                "Invalid component-ref id " + idAttribute.getValue() + " detected for a "
                                        + "remote resource. The " + "remote resource will not be used");
                        continue;
                    }
                    ...
                    if (remoteComponentFile != null) {
                        ...
                        Element remoteComponentContentElement
                                = remoteComponentContentDoc.getJDOMDocument().getRootElement();
                        ...
                        Element newComponent = new Element("component", scapVersion.getDSNamespace())
                                .setAttribute("id",
                                        new String(newComponentID + "_" + remoteComponentURL).replace("/", "_")
                                                .replace(":", "_"))
                        ...
                        ValidationNotes.getInstance().createValidationNote("Validation included remote "
                                + "component-ref with id " + idAttribute.getValue() + " and url " + remoteComponentURL);
                    } else {
                        ...
                        ValidationNotes.getInstance()
                                .createValidationNote("Unable to download " + "component-ref with id "
                                        + idAttribute.getValue() + " and url " + xlinkHrefAttribute.getValue());
                        continue;
                    }
                    ...
                    ValidationNotes.getInstance().createValidationNote(
                            "Unable to utilize remote " + "component-ref with id " + idAttribute.getValue()
     ...
     *            an int (in MiB) of the maximum external file download size supported, not null
     */
    public static Map<String, String> getXCCDF11RemoteResources(File xccdfFile, File scapContentsDir,
            int maxDownloadSize) throws SCAPException {
        ...
        String xccdfRemoteCheckRefsXpath = "//*[namespace-uri()=\"http://checklists.nist" + "" + ".gov/xccdf/1.1\" and "
                + "local-name()=\"Rule\"]//*[namespace-uri()=\"http://checklists.nist"
        ...
        // attempt to resolve any scap remote component resources
        List<Attribute> componentRemoteRefs
                = XMLUtils.getXpathAttributes(xccdfComponent, xccdfRemoteCheckRefsXpath);
        for (Attribute attribute : componentRemoteRefs) {
            ...
            if (contentRef.startsWith("http")) {
                ...
                try {
                    ...
                    File remoteComponentFile
                            = FileUtils.downloadFile(remoteComponentURL, maxDownloadSize * 1024 * 1024);
