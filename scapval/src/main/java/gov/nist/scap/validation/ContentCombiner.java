 ...
 * combines SCAP 1.1 content into a single file for validation.
 */
...
public class ContentCombiner {
   ...
   * for validation in SCAPVal. This is used for SCAP 1.1 content only.
   * Including the sourceDS is optional otherwise the parameter can be null
   *
   * @param scapContentsDir a File object of the directory containing the SCAP 1.1 files, not null
   * @param scapUseCase     the SCAP 1.1 use case under assessment, not null
   * @param maxDownloadSize an int (in MiB) of the maximum external file download size supported,
   *                        not null
   * @param isOnline        a boolean of the online status of this validation, not null
   * @param sourceDS        an XMLDocument of source content that will be included in the
   *                        combined result
  ...
  public static XMLDocument combineSCAP11(File scapContentsDir, String scapUseCase, int
    ...
    for (File file : scap11Files) {
      ...
      log.info("Found Component File " + file + "(SHA-256: " + FileUtils.getFileHash(file,
  ...
  /**
   * Determines exactly where the SCAP bundle is located.
  ...
  public static File locateSCAP11Bundle(String scapContentsDir) {
    ...
    // once the first valid SCAP 1.1 file is found that directory will will be considered our bundle
    Objects.requireNonNull(scapContentsDir, "scapContentsDir cannot be null.");
   ...
   *
   * @param bundleDir a File object of the directory containing SCAP 1.1 files, not null
      ...
      if (component.getAttribute("href", NS_XLINK.getNamespace()) != null) {
        ...
        if (xlinkHrefAttribute.getValue().startsWith("http")) {
          ...
          try {
            ...
            new URL(xlinkHrefAttribute.getValue());
